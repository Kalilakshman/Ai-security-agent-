...
class BurpSuiteAdapter(BaseToolAdapter):
    ...
    def is_installed(self) -> bool:
        import shutil
        if shutil.which("burpsuite") or shutil.which("burp"):
            return True
        ...
        try:
            with httpx.Client(timeout=3.0) as client:
                ...
                if resp.status_code in (200, 401, 404, 405, 500):
                    return f"Burp API Daemon Online ({self.api_url})"
        except Exception:
            pass
        return "Installed (Burp Binary)" if self.is_installed() else "Not Installed"
